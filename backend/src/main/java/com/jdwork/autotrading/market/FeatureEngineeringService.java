...
public class FeatureEngineeringService {
    ...
    /** 지정 종목의 최신 기준일 피처를 계산해 저장한다. 데이터가 전혀 없으면 아무것도 하지 않는다. */
    public void computeAndSave(String stockCode) {
        ...
        List<Double> closes = chronological.stream().map(bar -> bar.close().doubleValue()).toList();

        FeatureDaily feature = new FeatureDaily();
        feature.setStockCode(stockCode);
        feature.setBaseDate(chronological.get(chronological.size() - 1).tradeDateTime().toLocalDate());
        ...
        featureDailyMapper.upsert(feature);
    }
